...
// CODE HERE
const changeBackgroundColorBack = event => {
...
const repeatMyParam = (param) => {
    axios.get(`http://localhost:3000/repeat/HelloThere`)
    .then(res => {
        console.log(res.data)
        param.target.textContent = res.data;
...
const queryRequest = event => {
    axios.get('http://localhost:3000/query-test/?test&ok')
...
// You sent query: Testing with value: !
...
const createFood = event => {
    ...
    axios.post('http://localhost:3000/food', body)
        .then(response => {
            console.log(response.data)
            const para = document.createElement('p');
            para.textContent = response.data[response.data.length - 1];
            document.body.appendChild(para);
        })
        .catch(err => console.error(err))
    foodInput.value = '';
